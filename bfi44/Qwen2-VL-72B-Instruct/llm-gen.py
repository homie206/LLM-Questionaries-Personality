...
def generateResponse(client, gen_prompt, question):
    chat_response = client.chat.completions.create(
...
if __name__ == '__main__':
    ...
    for ipip_item in prompt_template["ipip50_prompt"]:
        ...
        output_file_name = f'llm-gen-result/{ipip_label_content_str}-combine-bfi44-qwen2-vl-72b-instruct-output.txt'
        result_file_name = f'llm-gen-result/{ipip_label_content_str}-combine-bfi44-qwen2-vl-72b-instruct-result.csv'
        ...
        with open(output_file_name, 'a', encoding='utf-8') as f, open(result_file_name, 'a', encoding='utf-8') as r:
            with open('../bfi44.txt', 'r') as f2:
                ...
                for run in range(20):  # 运行100次
                    extracted_numbers = []
                    gen_prompt = generatePrompt(client, ipip_prompt)

                    for q in question_list:
                        generated_text = generateResponse(client, gen_prompt, q)

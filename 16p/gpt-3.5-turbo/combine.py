...
def get_model_examing_result():
    ...
    for mbti_item in prompt_template["mbti_prompt"]:
        ...
        output_file_name = f'combine-result/{mbti_label_content}-combine-mbti-gpt3.5-turbo-output.txt'
        result_file_name = f'combine-result/{mbti_label_content}-combine-mbti-gpt3.5-turbo-result.csv'
                ...
                for question_num, question in mbti_questions.items():
                    response = client.chat.completions.create(
                        ...
                        messages=[
                            ...
                            {
                                "role": "user",
                                "content": "You will be presented a statement to describe you. Please show the extent of how you agree the statement on a scale from 1 to 7, with 1 being agree and 7 being disagree. You can only reply a number from 1 to 7. Here is the statement: " + question
                            }
                        ],
                        #temperature=0.6,
                        #max_tokens=256,
...
def get_single_model_examing_result():

    for mbti_item in single_prompt_template["mbti_prompt"]:
        mbti_prompt = mbti_item["prompt"]
        mbti_label_content = mbti_item["label"]

        output_file_name = f'/Users/haoming/Desktop/gpt_16p/LLM-Personality-Questionnaires/16p/gpt/combined-result/{mbti_label_content}-combined-mbti-gpt3.5-turbo-output.txt'
        result_file_name = f'/Users/haoming/Desktop/gpt_16p/LLM-Personality-Questionnaires/16p/gpt/combined-result/{mbti_label_content}-combined-mbti-gpt3.5-turbo-result.csv'

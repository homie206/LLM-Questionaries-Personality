...
def generatePrompt(client, prompt):
...
def get_model_examing_result(client):
    ...
    for mbti_item in prompt_template["mbti_prompt"][4:]:
        ...
        output_file_name = f'llm-gen-result/{mbti_label_content}-gen-induce-mbti-qwen2-vl-72b-instruct-output.txt'
        result_file_name = f'llm-gen-result/{mbti_label_content}-gen-induce-mbti-qwen2-vl-72b-instruct-result.csv'
        ...
        with open(output_file_name, 'a', encoding='utf-8') as f, open(result_file_name, 'a',
            ...
            for cycle in range(1, 21):
                ...
                generated_prompt = generatePrompt(client, mbti_prompt)
                for question_num, question in mbti_questions.items():

                    generated_text = generateResponse(client, generated_prompt, question)

                    f.write(f"cycle: {cycle}\n")
                    print(f"cycle: {cycle}\n")
                    f.write(f"Imagine you are a human with following personality: " + generated_prompt + "\n")
                    print(f"Imagine you are a human with following personality: " + generated_prompt + "\n")
                    ...
                    f.write(f"response: {generated_text}\n")
